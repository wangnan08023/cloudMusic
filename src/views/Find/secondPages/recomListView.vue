<template>
  <div class="recom-container">
    <!-- 播放器部分 -->
    <!-- <PlayerComp ref="player" :show="showPlay" class="PlayerComp"></PlayerComp> -->
    <!-- 歌单部分 -->
    <transition name="recom">
      <div class="recom">
        ...
        <div class="box-list">
          ...
          <ul class="songlist">
            <li v-for="(item,index) in songs" :key="item.id" @click="isShowPlay(index,songs)">
              <span class="id">{{index+1}}</span>
              <div class="name">
                <p>{{item.name}}</p>
                <p>
                  <span v-for="item1 in item.ar" :key="item1.id" class="author">
                    {{item1.name}}
                  </span>
                </p>
              </div>
              <span class="iconfont icon-caozuo-bofang"></span>
              <span class="iconfont icon-qita"></span>
            </li>
          </ul>
        </div>
...
export default {
  ...
  created() {
    // 获取二次页面-当前歌单详情
    this.$store.dispatch('getRecomListDetail', this.$route.query.listId)
    // 获取二次页面-当前歌单所有歌曲
    this.$store.dispatch('getRecomListSongs', this.$route.query.listId)
